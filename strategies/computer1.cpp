...
random_strategy_t::random_strategy_t(std::string name) :
        name(std::move(name)) {}

step_t random_strategy_t::make_step(const field_t &fld, const size_t player_num) {
...
void random_strategy_t::print_stat() const {
    std::cout << "Random model [" << name << "]: " << std::endl;
    computer_interface_t::print_stat();
}

std::vector<step_t> random_strategy_t::find_moves(const field_t &field, const size_t player_num) {
    char mark;
    char queen_mark;
